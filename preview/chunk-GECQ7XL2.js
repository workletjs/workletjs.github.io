...
`;function de(c){let r=W(),e=[];if(c.color!==void 0){let u=j(r,c.color,X);e.push(`color = ${u};`)}let t=Object.keys(r.variables);if(t.length>1&&!c.variables)throw new Error(`Missing variables in style (expected ${r.variables})`);let o={};for(let u of t){if(!(u in c.variables))throw new Error(`Missing '${u}' in style variables`);let h=B(u);o[h]=function(){let g=c.variables[u];return typeof g=="string"&&(g=G(g)),g!==void 0?g:-9999999}}let n=Object.keys(o).map(function(u){return`uniform float ${u};`}),a=Object.keys(r.functions).map(function(u){return r.functions[u]}),f=`
    #ifdef GL_FRAGMENT_PRECISION_HIGH
    precision highp float;
    #else
    precision mediump float;
