...
}`;let n=i["stroke-line-dash"].map(p=>m(t,p,v));n.length%2===1&&(n=[...n,...n]);let r="0.";"stroke-line-dash-offset"in i&&(r=m(t,i["stroke-line-dash-offset"],v));let a=`dashDistanceField_${Be(i["stroke-line-dash"])}`,c=n.map((p,w)=>`float dashLength${w}`).join(", "),l=n.map((p,w)=>`dashLength${w}`).join(" + "),u="0.",h=`getSingleDashDistance(distance, radius, ${u}, dashLength0, totalDashLength, capType, lineWidth)`;for(let p=2;p<n.length;p+=2)u=`${u} + dashLength${p-2} + dashLength${p-1}`,h=`min(${h}, getSingleDashDistance(distance, radius, ${u}, dashLength${p}, totalDashLength, capType, lineWidth))`;t.functions[a]=`float ${a}(float distance, float radius, float capType, float lineWidth, ${c}) {
  float totalDashLength = ${l};
  return ${h};
}`;let d=n.map((p,w)=>`${p}`).join(", ");e.setStrokeDistanceFieldExpression(`${a}(currentLengthPx + ${r}, currentRadiusPx, capType, v_width, ${d})`)}}function Yo(i,e,o,t){if("fill-color"in i&&e.setFillColorExpression(m(t,i["fill-color"],K)),"fill-pattern-src"in i){let n=Be(i["fill-pattern-src"]),r=at(i,e,o,"fill-pattern-",n),s=r,a="vec2(0.)";"fill-pattern-offset"in i&&"fill-pattern-size"in i&&(s=m(t,i["fill-pattern-size"],te),a=lt(i,"fill-pattern-",t,r,s)),t.functions.sampleFillPattern=`vec4 sampleFillPattern(sampler2D texture, vec2 textureSize, vec2 textureOffset, vec2 sampleSize, vec2 pxOrigin, vec2 pxPosition) {
